...
Status SrSetPropertyType(SrPropertyT *property, const char *type) {
    if (strcmp(type, "int") == 0) {
        ...
        log_error("SrSetPropertyType: type is not support.");
...
Status SrParseSingleField(json_t *field, SrPropertyT *property) {
    ...
    ret = SrSetPropertyType(property, fieldTypeStr);
    if (ret != GMERR_OK) {
        log_error("SrSetPropertyType failed.");
        return ret;
    }
    // 解析字段长度
...
void InitLabelInfo(SrLabelT *label, SeLabelInfoT *labelInfo) {
    DB_POINT2(label, labelInfo);
    labelInfo->dbId = label->dbId;
    labelInfo->labelId = label->labelId;
    labelInfo->recordLen = label->recordLen;
}
...
Status DMSrCreateTable(QryStmtT *stmt) {
    ...
    InitLabelInfo(&labelCtrl, &labelInfo);
    ret = EECreateLabelContainer(&labelInfo);
    if (ret != GMERR_OK) {
